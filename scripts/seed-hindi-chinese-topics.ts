import { prisma } from "../lib/prisma";
import { generateSlug } from "../lib/slug";

async function main() {
  // Buscar o primeiro user (ou o user específico)
  const user = await prisma.user.findFirst({
    orderBy: { createdAt: "desc" },
  });

  if (!user) {
    throw new Error("Nenhum user encontrado! Cria um user primeiro.");
  }

  console.log(`\n🎯 A criar tópicos para: ${user.username} (${user.email})`);
  console.log(`   User ID: ${user.id}\n`);

  const topics = [
    // Hindi (hi) - 10 tópicos polêmicos
    {
      title: "क्या कृत्रिम बुद्धिमत्ता मानव नौकरियों को खत्म कर देगी?",
      description:
        "AI और ऑटोमेशन का बढ़ता उपयोग - क्या यह रोजगार के लिए खतरा है या नए अवसर खोलेगा?",
      language: "hi" as const,
    },
    {
      title: "सोशल मीडिया लोकतंत्र के लिए अच्छा है या बुरा?",
      description: "सोशल मीडिया ने राजनीतिक चर्चा को कैसे बदल दिया है - सकारात्मक या नकारात्मक?",
      language: "hi" as const,
    },
    {
      title: "क्या क्रिप्टोकरेंसी पारंपरिक मुद्रा को प्रतिस्थापित कर सकती है?",
      description:
        "Bitcoin और अन्य क्रिप्टोकरेंसी - क्या यह भविष्य की मुद्रा है या सिर्फ सट्टेबाजी?",
      language: "hi" as const,
    },
    {
      title: "दूरस्थ कार्य बेहतर है या कार्यालय में काम?",
      description: "COVID के बाद, कंपनियां कर्मचारियों को वापस बुला रही हैं - क्या यह सही है?",
      language: "hi" as const,
    },
    {
      title: "जलवायु परिवर्तन: व्यक्तिगत कार्रवाई पर्याप्त है?",
      description:
        "क्या व्यक्तिगत प्रयास जलवायु संकट को हल कर सकते हैं या केवल सरकारी नीतियां काम करेंगी?",
      language: "hi" as const,
    },
    {
      title: "ऑनलाइन शिक्षा पारंपरिक शिक्षा जितनी प्रभावी है?",
      description: "डिजिटल प्लेटफॉर्म पर शिक्षा - क्या यह कक्षा शिक्षा का वास्तविक विकल्प है?",
      language: "hi" as const,
    },
    {
      title: "शाकाहार स्वास्थ्य और पर्यावरण के लिए बेहतर है?",
      description: "मांसाहार बनाम शाकाहार - वैज्ञानिक और नैतिक तर्क क्या कहते हैं?",
      language: "hi" as const,
    },
    {
      title: "निजता बनाम सुरक्षा: हमें कितना समझौता करना चाहिए?",
      description:
        "सरकारी निगरानी और डेटा संग्रह - सुरक्षा के लिए कितनी निजता का त्याग स्वीकार्य है?",
      language: "hi" as const,
    },
    {
      title: "तकनीक बच्चों के विकास को कैसे प्रभावित करती है?",
      description: "स्मार्टफोन और टैबलेट का बच्चों पर प्रभाव - हानिकारक या लाभदायक?",
      language: "hi" as const,
    },
    {
      title: "यूनिवर्सल बेसिक इनकम एक व्यावहारिक समाधान है?",
      description: "सभी नागरिकों को मुफ्त आय - क्या यह आर्थिक असमानता को हल कर सकता है?",
      language: "hi" as const,
    },
    // Chinese (zh) - 10 tópicos polêmicos
    {
      title: "人工智能会取代人类工作吗？",
      description: "AI和自动化的快速发展 - 这对就业是威胁还是会创造新机会？",
      language: "zh" as const,
    },
    {
      title: "社交媒体对民主是好是坏？",
      description: "社交媒体如何改变政治讨论 - 是积极的还是消极的影响？",
      language: "zh" as const,
    },
    {
      title: "加密货币能取代传统货币吗？",
      description: "比特币和其他加密货币 - 这是未来的货币还是只是投机？",
      language: "zh" as const,
    },
    {
      title: "远程工作更好还是办公室工作？",
      description: "疫情后，公司要求员工返回办公室 - 这是正确的决定吗？",
      language: "zh" as const,
    },
    {
      title: "气候变化：个人行动足够吗？",
      description: "个人努力能解决气候危机吗，还是只有政府政策才能奏效？",
      language: "zh" as const,
    },
    {
      title: "在线教育和传统教育一样有效吗？",
      description: "数字平台上的教育 - 这是课堂教育的真正替代方案吗？",
      language: "zh" as const,
    },
    {
      title: "素食对健康和环境更好吗？",
      description: "肉食与素食 - 科学和伦理论据说了什么？",
      language: "zh" as const,
    },
    {
      title: "隐私与安全：我们应该妥协多少？",
      description: "政府监控和数据收集 - 为了安全放弃多少隐私是可以接受的？",
      language: "zh" as const,
    },
    {
      title: "技术如何影响儿童发展？",
      description: "智能手机和平板电脑对儿童的影响 - 有害还是有益？",
      language: "zh" as const,
    },
    {
      title: "全民基本收入是可行的解决方案吗？",
      description: "给所有公民提供免费收入 - 这能解决经济不平等吗？",
      language: "zh" as const,
    },
  ];

  console.log(`📝 A criar ${topics.length} tópicos...\n`);

  let created = 0;
  const errors: Array<{ title: string; error: string }> = [];

  for (const topic of topics) {
    try {
      const slug = generateSlug(topic.title);
      await prisma.topic.create({
        data: {
          title: topic.title,
          description: topic.description,
          language: topic.language,
          slug,
          status: "ACTIVE",
          createdById: user.id,
        },
      });
      created++;
      console.log(`✅ [${topic.language}] ${topic.title}`);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "Erro desconhecido";
      errors.push({ title: topic.title, error: errorMessage });
      console.log(`❌ [${topic.language}] ${topic.title} - ${errorMessage}`);
    }
  }

  console.log(`\n✨ Resumo:`);
  console.log(`   ✅ Criados: ${created}`);
  console.log(`   ❌ Erros: ${errors.length}`);

  if (errors.length > 0) {
    console.log(`\n⚠️  Erros encontrados:`);
    errors.forEach((e) => {
      console.log(`   - ${e.title}: ${e.error}`);
    });
  }

  // Contagem por idioma
  const counts = await prisma.topic.groupBy({
    by: ["language"],
    _count: true,
  });

  console.log(`\n📊 Tópicos por idioma:`);
  counts.forEach((c) => {
    console.log(`   ${c.language}: ${c._count}`);
  });
}

main()
  .catch((e) => {
    console.error("❌ Erro fatal:", e);
    process.exit(1);
  })
  .finally(async () => {
    await prisma.$disconnect();
  });
